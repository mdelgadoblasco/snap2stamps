...
parser = argparse.ArgumentParser(description='This script runs the DEM Assisted coregstration using External DEM')
...
outlog=os.path.join(logfolder,'coreg_proc_stdout.log')

# Original Snap graph and replaced Snap graph for Coregistration 
if extDEM=='':
    graphxml=os.path.join(GRAPH,'stripmap_DEM_Assisted_Coregistration.xml')
else:
    graphxml=os.path.join(GRAPH,'stripmap_DEM_Assisted_Coregistration_extDEM.xml')
print(graphxml)
graph2run=os.path.join(GRAPH,'coreg_2run.xml')
...
for dimfile in glob.iglob(subsetfolder + '/*.dim'):
    ...
    print(message)
    out_file.write(message)
    head , tailm = os.path.split(masterfolder)
    ...
    filedata = filedata.replace('MASTER',masterfolder)
    ...
    if process.returncode != 0 :
        message='Error computing with coregistration of splitted secondaries '+str(dimfile)
