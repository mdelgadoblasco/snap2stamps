...
splitmasterfolder=os.path.join(PROJECT,'MasterSplit')
...
outlog=os.path.join(logfolder,'splitmaster.log')
...
files = glob.glob(masterfolder + '/*.zip')
filename=os.path.basename(files[0])
acdatefolder=filename[17:25]
if len(files)==0:
    files = glob.glob(os.path.join(masterfolder, acdatefolder) + '/*SAFE/manifest.safe')
print (files)
...
for IW in IWlist:
    ...
    print('Splitting master with AOI intersecting '+IW)
    if len(files) == 1 :
        graphxml=os.path.join(GRAPH,'topsar_master_split_applyorbit.xml')
        # Read in the file
        ...
        filedata = filedata.replace('OUTPUTFILE',os.path.join(splitmasterfolder,outputname))
        # # Write the file out again
        with open(graph2run, 'w') as file:
            file.write(filedata)
    if len(files) == 2 :
        graphxml=os.path.join(GRAPH,'topsar_master_assemble_split_applyorbit.xml')
        with open(graphxml, 'r') as file :
            filedata = file.read()
            # Replace the target string
    ...
    print('['+IW+'] Finished process in '+str(timeDelta)+' seconds.')
    out_file.write('['+str(IW)+'] Finished process in '+str(timeDelta)+' seconds.\n')
    if process.returncode != 0 :
        message='Master image does not intersect '+str(polygon)+' in subswath '+IW
        err_file.write(message)
    else: 
        message='Split master '+str(files)+' in subswath '+IW+' successfully completed.\n'
    print( message)
    out_file.write(message)
    print( bar_message)
    out_file.write(bar_message)
...
# Updating project.conf file with new Master splitted
#with open(inputfile, 'r') as file :
#	filedata = file.read()
# Replace the target string
#filedata = filedata.replace('MASTER=','MASTER='+splitmasterfolder+'/'+outputname)
# Write the file out again
