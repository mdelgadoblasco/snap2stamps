...
***The provided scripts with this SNAP to StaMPS package are not part of the SNAP software. The provided scripts are open-source contributions and should not be mistaken for official Applications within SNAP. The scripts are provided to you "as is" with no warranties of corrections. Use at your own risk.***
...
## Description
SNAP2StaMPS version 2 tries to collect feedback and it has been built to minimize issues from the user side due to package installation, or other dependencies due to software versioning, etc. Further, the new version supports the preprocessing of high-resolution TerraSAR-/TanDEM-X Stripmap data (TSX2Stamps) for the ingestion in StaMPS. TSX2Stamps uses a similar workflow applied to Sentinel-1 Topsar data (Sentinel2Stamps) in the previous version of SNAP2StaMPS. The TSX2Stamps package is already freely available for download on GitHub (https://github.com/jziemer1996/TSX2StaMPS). Please find below the general workflow of SNAP2StaMPS_v2. For further information, please check the information given below or have a look at the tutorial provided within this repository.
...
2. Advanced execution using dockerised software
...
Due to the integration of Stripmap data, the new SNAP2StaMPS package consists of two separate branches (Sentinel2Stamps and TSX2Stamps) with two different config files (project_topsar.conf and project_stripmap.conf). Please spend some time understanding the (new) options available in the configuration files.
...
Since TSX/TDX is not freely available, the download and auto-master selection is only available for Sentinel-1 data. Consequently, the configuration file of stripmap data comes without these parameters and is slightly different.
...
# Contributing
- José Manuel Delgado Blasco (University of Jaen, Spain)
